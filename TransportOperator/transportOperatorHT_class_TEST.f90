...
contains
  ...
  subroutine tracking_selection(self, p, tally, thisCycle, nextCycle)
    ...
    HTLoop:do
      ...
      if (ratio > (ONE - self % cutoff) .and. p % matIdx() /= VOID_MAT) then
        call deltaTracking(self, p, tally, thisCycle, nextCycle, event)
      else
        call surfaceTracking(self, p, tally, thisCycle, nextCycle, event)
      end if

      if (event == COLL_EV .or. p % isDead) exit HTLoop
  ...
  !!
  subroutine deltaTracking(self, p, tally, thisCycle, nextCycle, event)
    class(transportOperatorHT), intent(inout) :: self
    class(particle), intent(inout)            :: p
    type(tallyAdmin), intent(inout)           :: tally
    class(particleDungeon), intent(inout)     :: thisCycle
    class(particleDungeon), intent(inout)     :: nextCycle
    integer(shortInt), intent(inout)          :: event
  ...
  !!
  subroutine surfaceTracking(self, p, tally, thisCycle, nextCycle, event)
    class(transportOperatorHT), intent(inout) :: self
    class(particle), intent(inout)            :: p
    type(tallyAdmin), intent(inout)           :: tally
    class(particleDungeon),intent(inout)      :: thisCycle
