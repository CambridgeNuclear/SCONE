...
@testCase
  type, extends(TestCase) :: test_timeMap
    private
    type(timeMap) :: map_lin
    type(timeMap) :: map_unstruct
  contains
    procedure :: setUp
    procedure :: tearDown
  end type test_timeMap
...
contains
  ...
  !!
  !! Sets up test_timeMap object we can use in a number of tests
  !!
  subroutine setUp(this)
    class(test_timeMap), intent(inout) :: this
  ...
  !!
  !! Kills test_timeMap object we can use in a number of tests
  !!
...
@Test
  subroutine testLinearGrid(this)
    class(test_timeMap), intent(inout)   :: this
    real(defReal),dimension(6),parameter :: T = [7.5774_defReal, 9.3652_defReal, 3.9223_defReal, &
...
@Test
  subroutine testUnstructGrid(this)
    class(test_timeMap), intent(inout)   :: this
    real(defReal),dimension(6),parameter :: T = [0.0761191517392624_defReal,   &
