module materialMemoryMT_class
  ...
  type, public :: materialMemoryMT
    ...
    type(nuclideMemoryMT),dimension(:),pointer :: nucShelf => null()  !! Pointer to microscopic xss for all nuclides
...
contains
  ...
  subroutine init(self,dataPtr,nucPtr)
    class(materialMemoryMT), intent(inout)                :: self
    type(materialDataNoMT), pointer, intent(in)             :: dataPtr
    type(nuclideMemoryMT),dimension(:),pointer,intent(in) :: nucPtr
    character(100), parameter                 :: Here ='init (materialMemoryMT_class.f90)'
...
end module materialMemoryMT_class
