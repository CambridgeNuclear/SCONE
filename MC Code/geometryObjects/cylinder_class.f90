...
module cylinder_class
  ...
  type, public, extends(surface) :: xCylinder
    real(defReal) :: rSquared = 0.0                         ! squared radius of the cylinder
    real(defReal) :: radius = 0.0                           ! radius of the cylinder
    real(defReal), dimension(3) :: origin = [0.0, 0.0, 0.0] ! 2D origin of cylinder assumed parallel to x-axis
  contains
  ...
  type, public, extends(surface) :: yCylinder
    real(defReal) :: rSquared = 0.0                         ! squared radius of the cylinder
    real(defReal) :: radius = 0.0                           ! radius of the cylinder
    real(defReal), dimension(3) :: origin = [0.0, 0.0, 0.0] ! 2D origin of cylinder assumed parallel to y-axis
  contains
  ...
  type, public, extends(surface) :: zCylinder
    real(defReal) :: rSquared = 0.0                         ! squared radius of the cylinder
    real(defReal) :: radius = 0.0                           ! radius of the cylinder
    real(defReal), dimension(3) :: origin = [0.0, 0.0, 0.0] ! 2D origin of cylinder assumed parallel to z-axis
  contains
...
!
  ...
  function distanceToXCyl(self, r, u) result(distance)
    ...
    k = yBar*u(2) + zBar*u(3)
    a = 1.0 - u(1)*u(1) ! = u(2)*u(2) + u(3)*u(3)
    c = yBar*yBar + zBar*zBar - self%rSquared
    discriminant = k*k - a*c

    ! No intersection
    if ((a==0.0).OR.(discriminant<0.0)) then
      distance = INFINITY
      return
  ...
  subroutine reflectiveTransformXCyl(self,r,u)
    ...
    call fatalError('reflectiveTransformXCyl','Cylinders may not have reflective boundaries')
  ...
  function normalVectorXCyl(self,r)result(normal)
    ...
    normal = [0.0_8, 2.0*(r(2) - self%origin(2)), 2.0*(r(3) - self%origin(3))]
    return
  ...
  function whichXCylinder(self, r, u) result(surfPointer)
    ...
    call fatalError('whichXCylinder','This function should never be called for a simple surface')
...
!
  ...
  subroutine reflectiveTransformYCyl(self,r,u)
    ...
    ! Reflective transforms will not be allowed to occur in geometries other than planes
    call fatalError('reflectiveTransformYCyl','Cylinders may not have reflective boundaries')
  ...
  function normalVectorYCyl(self,r)result(normal)
    ...
    normal = [2.0*(r(1) - self%origin(1)), 0.0_8, 2.0*(r(3) - self%origin(3))]
    return
...
!
  ...
  subroutine reflectiveTransformZCyl(self,r,u)
    ...
    ! Reflective transforms will not be allowed to occur in geometries other than planes
    call fatalError('reflectiveTransformZCyl','Cylinders may not have reflective boundaries')
  ...
  function normalVectorZCyl(self,r)result(normal)
    ...
    normal = [2.0*(r(1) - self%origin(1)), 2.0*(r(2) - self%origin(2)), 0.0_8]
    return
  ...
  function whichZCylinder(self, r, u) result(surfPointer)
    ...
    real(defReal), dimension(3), intent(in) :: r, u
    class(surface), pointer :: surfPointer
    call fatalError('whichZCylinder','This function should never be called for a simple surface')
