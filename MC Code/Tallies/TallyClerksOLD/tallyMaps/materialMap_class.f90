...
contains
  ...
  !!
  !! Initialise material map
  !!
  subroutine init(self, materials, trackRest)
  ...
  function map(self,p) result(idx)
    ...
    idx = self % binMap % getOrDefault( p % matIdx(), self % default)
  ...
  function materialMap_fromDict(dict) result(new)
    ...
    ! Initialise Map
    call new % init(matNames, trackUndefined)
