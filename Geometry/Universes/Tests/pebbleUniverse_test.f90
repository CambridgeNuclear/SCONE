module pebbleUniverse_test
  ...
  use pebbleUniverse_class, only : pebbleUniverse, MOVING_IN, MOVING_OUT
  use funit
  implicit none

  ! Parameters
  character(*), parameter :: UNI_DEF = &
  "id 7; type pebbleUniverse; origin (0.0 0.0 0.0); &
...
@Test
  subroutine test_enter()
    ...
    r = [2.3_defReal, 0.0_defReal, 0.0_defReal ]
    dir = [ZERO, ZERO, ONE]
    ...
    r = [0.0_defReal, 0.0_defReal, 2.6_defReal ]
    dir = [ZERO, ONE, ZERO]
    ...
    ! Verify that the universe is not rotated
...
@Test
  subroutine test_distance()
    ...
    pos % r = [0.0_defReal, 0.0_defReal, 1.0_defReal]
    ...
    pos % r = [2.0_defReal, 0.0_defReal, 1.6_defReal]
    pos % dir = [ZERO, ZERO, ONE]
    pos % localId = 3
...
@Test
  subroutine test_cross()
    ...
    eps = HALF * SURF_TOL
    pos % r   = [0.0_defReal, 0.0_defReal, 1.5_defReal+eps]
    pos % dir = [ZERO, ZERO, -ONE]
...
@Test
  ...
  !! if particle is placed very close to a sphere surface (within SURF_TOL)
  !!
@Test
...
@Test
  subroutine test_normal()
    ...
    ! Surface should be innermost sphere
    idx = MOVING_IN
    n = uni % getNormal(idx, pos)
    @assertEqual([0, -1, 0], n)
    
    ! Surface should be second innermost sphere - results will be identical
    ...
    ! And try z-axis
    pos % r   = [0.0_defReal, 0.0_defReal, 9999.9_defReal]
    pos % dir = [ZERO, ONE, ZERO]
    pos % localId = 3
    idx = MOVING_OUT
    n = uni % getNormal(idx, pos)
    ...
    ! And finally a combination
    pos % r   = [0.0_defReal, -9999.9_defReal, 9999.9_defReal]
    pos % dir = [ZERO, ONE, ZERO]
    pos % localId = 3
    idx = MOVING_OUT
    n = uni % getNormal(idx, pos)
    @assertEqual([ZERO, -sqrt(TWO) * HALF, sqrt(TWO) * HALF], n, 1.0E-7_defReal)
